...
class Config:
    ...
    def load(s: QtCore.QSettings) -> "Config":
        cfg = Config()
        cfg.color_start = Config._hex_to_qcolor(s.value("color_start", "#F05A28"), QtGui.QColor(240, 90, 40))
        cfg.color_end   = Config._hex_to_qcolor(s.value("color_end",   "#FBCA0A"), QtGui.QColor(251, 202, 10))
...
class Tray(QtWidgets.QSystemTrayIcon):
    def __init__(self, overlay: Overlay, settings: QtCore.QSettings, parent=None):
        ...
        # in your Tray.__init__ (replace the generated icon)
        ico_file = asset_path("Resources", "graftrail.ico")
        if os.path.exists(ico_file):
            self.setIcon(QtGui.QIcon(ico_file))
        else:
            self.setIcon(self._default_icon())  # fallback
